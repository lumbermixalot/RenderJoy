...
namespace RenderJoy
{
    ITextureWriter::ITextureWriter(uint16_t mipLevels, AZ::RHI::Format pixelFormat,
    ...
    bool ITextureWriter::SetDataBufferForMipLevel(AZStd::shared_ptr<AZStd::vector<uint8_t>> dataBuffer,
    ...
    void ITextureWriter::SetMipLevelSaved(uint16_t mipLevel)
    {
        m_savedMipLevels[mipLevel] = true;
    }

    uint32_t ITextureWriter::GetBytesPerRow(uint16_t mipLevel) const
    {
        const auto layout = AZ::RHI::GetImageSubresourceLayout(m_mipLevelsDataList[mipLevel].m_mipSize, m_pixelFormat);
        return layout.m_bytesPerRow;
    }
