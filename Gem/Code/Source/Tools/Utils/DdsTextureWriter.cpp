...
#include "DdsTextureWriter.h"

namespace RenderJoy
{
    DdsTextureWriter::DdsTextureWriter(uint16_t mipLevels, AZ::RHI::Format pixelFormat, const AZ::IO::Path& outputDir, const AZStd::string& stemPrefix)
        : ITextureWriter(mipLevels, pixelFormat, outputDir, stemPrefix)
    {

    }


    DdsTextureWriter::~DdsTextureWriter()
    {
    ...
    uint32_t DdsTextureWriter::CalculatePixelBufferSize()
    {
    ...
    bool DdsTextureWriter::SaveMipLevel(uint16_t mipLevel, AZStd::vector<AZ::IO::Path>* savedFiles)
    {
