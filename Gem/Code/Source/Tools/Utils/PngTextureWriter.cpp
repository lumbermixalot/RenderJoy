...
#include "PngTextureWriter.h"

namespace RenderJoy
{
    PngTextureWriter::PngTextureWriter(uint16_t mipLevels, AZ::RHI::Format pixelFormat, const AZ::IO::Path& outputDir, const AZStd::string& stemPrefix)
    ...
    PngTextureWriter::~PngTextureWriter()
    {
    ...
    bool PngTextureWriter::SaveMipLevel(uint16_t mipLevel, AZStd::vector<AZ::IO::Path>* savedFiles)
    {
        ...
        const uint32_t bytesPerRow = GetBytesPerRow(mipLevel);
